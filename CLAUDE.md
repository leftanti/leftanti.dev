...
**No reserve left.** The hue freed by merging KQL and Rules into `DetectionEngineering` was claimed by `Labs`. Ten hues, all in use. [docs/palette.md](docs/palette.md) is the full reference: what every hue means, where colour is allowed to appear, the steps to add a section or a resource category, and the contrast and separation constraints a new hue must satisfy. **Read it before adding or retuning any colour.**
